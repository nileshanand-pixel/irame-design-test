...
export default function CommentForm({
    ...
    const { addFiles, files,removeFile, progress, resetUploads, isAllFilesUploaded, uploadedMetadata } = useFileUploadsV2();
    ...
    return (
        <form className={`bg-[#FAF5FF] p-3 flex flex-col gap-[10px] rounded-b-lg ${!isCommentSectionOpen && "rounded-t-lg"}`}>
            ...
            <div className="flex gap-3 flex-wrap">
                {
                    imageFiles.length !== 0 && imageFiles.map((imageFile) => (
                        <ImagePreview 
                            key={imageFile.name}
                            file={imageFile}
            ...
            <div className="flex gap-3 flex-wrap">
                {
                 notImageFiles.length !== 0 && notImageFiles.map((file) => (
                    <FilePreview
                        fileName={file.name}
                        handleCancel={removeFile}
