...
import { getEdaJobs, deleteEdaJob } from '../../service/eda.service';
...
const HistoryTab = ({ onViewJob }) => {
	...
	const handleDelete = async () => {
		...
		try {
			setIsDeleting(true);
			await deleteEdaJob(deleteJobId);
			toast.success('Job deleted successfully');
			queryClient.invalidateQueries({ queryKey: ['eda-jobs-history'] });
		} catch {
			...
			<Dialog open={!!deleteJobId} onOpenChange={() => setDeleteJobId(null)}>
				<DialogContent className="max-w-sm">
					...
					<p className="text-sm text-primary60 mt-2">
						This will permanently delete this analysis and its reports
