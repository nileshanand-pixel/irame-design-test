...
const PDFViewer = ({ fileUrl, isOpen, onClose, usedPages }) => {
  ...
  const renderPage = useCallback(
    (pageNumber) =>
      !(showOnlyUsedPages && !isPageUsed(pageNumber)) && (
        <div key={pageNumber} data-page-number={pageNumber} className="relative mb-4 bg-primary40 text-center">
          <Page
  ...
  const scrollToPage = (pageNumber) => {
    const pageElement = mainContentRef.current?.querySelector(`[data-page-number="${pageNumber}"]`);
    const thumbnailElement = thumbnailRef.current?.querySelector(`[data-thumb-number="${pageNumber}"]`);
  ...
  useEffect(() => {
    if (mainContentRef.current) {
      const handleIntersection = (entries) => {
        ...
        if (visiblePage) setCurrentPage(Number(visiblePage.target.getAttribute('data-page-number')));
      };
      ...
      const pages = mainContentRef.current.querySelectorAll('[data-page-number]');
  ...
  useEffect(() => {
    const thumbnailElement = thumbnailRef.current?.querySelector(`[data-thumb-number="${currentPage}"]`);
    thumbnailElement?.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'nearest' });
  }, [currentPage]);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      <DialogContent className="max-w-7xl h-[90vh] p-6">
          ...
          <div className="flex items-center gap-8">
            ...
            <label className="flex items-center ml-12 gap-2">
              <input
                ...
                onChange={(e) => setShowOnlyUsedPages(e.target.checked)}
        ...
        <div className="grid grid-cols-12 gap-4 h-full overflow-hidden">
          <div ref={thumbnailRef} className="col-span-2 overflow-y-auto border-2 bg-primary40/10 py-2 px-2">
                    ...
                    !showOnlyUsedPages || isPageUsed(pageNumber) ? (
                      <div
                        ...
                        className={`relative mx-2 mt-2 mb-4 cursor-pointer ${
                          currentPage === pageNumber ? 'ring-2 ring-cyan-800' : ''
                        }`}
          ...
          <div ref={mainContentRef} className="col-span-10 overflow-y-auto">
            <Document file={fileUrl} onLoadSuccess={onDocumentLoadSuccess}>
              {numPages &&
