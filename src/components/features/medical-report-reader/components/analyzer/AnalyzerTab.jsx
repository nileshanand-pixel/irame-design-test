...
import {
	createMedicalReaderJob,
	getMedicalReaderJobResult,
	getMedicalReaderJobStatus,
	deleteMedicalReaderJob,
	uploadMedicalReaderFilesLocal,
} from '../../service/medical-reader.service';
import { useMedicalReaderJobPolling } from '../../hooks/useMedicalReaderJobPolling';
...
const AnalyzerTab = ({ selectedJobId, onJobIdChange }) => {
	...
	const { data: statusData } = useMedicalReaderJobPolling(
		jobId,
	...
	useEffect(() => {
		if (selectedJobId && selectedJobId !== internalJobIdRef.current) {
			...
			setResult(null);
			getMedicalReaderJobStatus(selectedJobId)
				.then((status) => {
					if (status.status === 'COMPLETED') {
						...
						setErrorMessage(
							status.message || 'Analysis failed. Please try again.',
						);
	...
	useEffect(() => {
		...
		if (statusData.status === 'COMPLETED') {
			...
			setState(STATES.ERROR);
			setErrorMessage(
				statusData.message || 'Analysis failed. Please try again.',
			);
	...
	const loadResult = async (id) => {
		try {
			const data = await getMedicalReaderJobResult(id);
			setResult(data);
		...
		async (files) => {
			try {
				...
				if (isLocalEnv) {
					...
					const response = await uploadMedicalReaderFilesLocal(files);
					...
					setUploadProgress(100);
					const response = await createMedicalReaderJob(fileUrls, names);
					newJobId = response.job_id;
				}
				...
				setState(STATES.PROCESSING);
				toast.info('Medical report analysis started');
			} catch (error) {
				setState(STATES.ERROR);
				setErrorMessage(
