...
import MedicalReaderHeader from './components/MedicalReaderHeader';
import MedicalReaderTabs from './components/MedicalReaderTabs';
import AnalyzerTab from './components/analyzer/AnalyzerTab';
import HistoryTab from './components/history/HistoryTab';
import { MR_TABS } from './constants/medical-reader.constants';

const MedicalReportReaderPage = () => {
	...
	const activeTab = useMemo(() => {
		...
		return tabParam &&
			Object.values(MR_TABS)
				?.map((t) => t.value)
				.includes(tabParam)
			? tabParam
			: MR_TABS.ANALYZER.value;
	}, [searchParams]);
	...
	const setSelectedJobId = useCallback(
		(jobId) => {
			const tab = searchParams.get('tab') || MR_TABS.ANALYZER.value;
			if (jobId) {
	...
	const handleViewJob = useCallback(
		(jobId) => {
			setSearchParams(
				{ tab: MR_TABS.ANALYZER.value, jobId },
				{ replace: true },
			);
		},
		[setSearchParams],
			...
			<div className="bg-white/55 backdrop-blur-xl rounded-2xl shadow-[0_4px_16px_rgba(106,18,205,0.06),inset_0_1px_0_rgba(255,255,255,0.8)] border border-white/70 overflow-hidden flex-1 flex flex-col min-h-0">
				...
				>
					...
					<div className="flex-1 overflow-auto px-6 pt-4 pb-6">
						<TabsContent value={MR_TABS.ANALYZER.value} className="mt-0">
							<AnalyzerTab
						...
						<TabsContent value={MR_TABS.HISTORY.value} className="mt-0">
...
export default MedicalReportReaderPage;
