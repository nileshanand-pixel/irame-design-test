import { IA_STAGES, IA_STAGE_ORDER } from '../../constants/imageAnalytics.constants';

const PhaseStepIndicator = ({ currentStage, status }) => {
	const currentIndex = IA_STAGE_ORDER.indexOf(currentStage);
	const isCompleted = status === 'COMPLETED';

	return (
		<div className="flex items-center justify-between w-full px-2">
			{IA_STAGE_ORDER.map((stageKey, index) => {
				const stage = IA_STAGES[stageKey.toUpperCase()] || {
					label: stageKey,
					color: 'text-gray-500',
					bgColor: 'bg-gray-500',
				};
