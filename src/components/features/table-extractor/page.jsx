...
import TableExtractorHeader from './components/TableExtractorHeader';
import TableExtractorTabs from './components/TableExtractorTabs';
import GeneratorTab from './components/generator/GeneratorTab';
import HistoryTab from './components/history/HistoryTab';
import { TE_TABS } from './constants/table-extractor.constants';

const TableExtractorPage = () => {
	...
	const activeTab = useMemo(() => {
		...
		return tabParam &&
			Object.values(TE_TABS)
				?.map((t) => t.value)
				.includes(tabParam)
			? tabParam
			: TE_TABS.GENERATOR.value;
	}, [searchParams]);
	...
	const setSelectedJobId = useCallback(
		(jobId) => {
			const tab = searchParams.get('tab') || TE_TABS.GENERATOR.value;
			if (jobId) {
	...
	const handleViewJob = useCallback(
		(jobId) => {
			setSearchParams(
				{ tab: TE_TABS.GENERATOR.value, jobId },
				{ replace: true },
			);
		},
		[setSearchParams],
			...
			<div className="bg-white/55 backdrop-blur-xl rounded-2xl shadow-[0_4px_16px_rgba(106,18,205,0.06),inset_0_1px_0_rgba(255,255,255,0.8)] border border-white/70 overflow-hidden flex-1 flex flex-col min-h-0">
				...
				>
					...
					<div className="flex-1 overflow-auto px-6 pt-4 pb-6">
						...
						<TabsContent value={TE_TABS.HISTORY.value} className="mt-0">
...
export default TableExtractorPage;
