import {
	FORENSICS_STAGES,
	FORENSICS_STAGE_ORDER,
} from '../../constants/forensics.constants';

const PhaseStepIndicator = ({ currentStage, status }) => {
	const currentIndex = FORENSICS_STAGE_ORDER.indexOf(currentStage);
	const isCompleted = status === 'COMPLETED';

	return (
		<div className="flex items-center justify-between w-full px-2">
			{FORENSICS_STAGE_ORDER.map((stageKey, index) => {
				const stage = FORENSICS_STAGES[stageKey.toUpperCase()] || {
					label: stageKey,
					color: 'text-gray-500',
					bgColor: 'bg-gray-500',
				};
				...
				return (
					...
					>
						...
						{index < FORENSICS_STAGE_ORDER.length - 1 && (
							<div
