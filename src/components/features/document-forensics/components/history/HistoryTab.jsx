...
import { getForensicJobs, deleteForensicJob } from '../../service/forensics.service';
...
const HistoryTab = ({ onViewJob }) => {
	...
	const { data: jobs, isLoading } = useQuery({
		queryKey: ['forensic-jobs-history'],
		queryFn: getForensicJobs,
	...
	const handleDelete = async () => {
		...
		try {
			setIsDeleting(true);
			await deleteForensicJob(deleteJobId);
		...
		return (
			<div className="text-center py-12">
				<p className="text-primary40 text-sm">
					No forensic jobs yet. Run your first analysis from the Analyzer
					tab.
				</p>
			</div>
		);
			...
			<Dialog open={!!deleteJobId} onOpenChange={() => setDeleteJobId(null)}>
				<DialogContent className="max-w-sm">
					...
					<p className="text-sm text-primary60 mt-2">
						This will permanently delete this analysis and cannot be
						undone. Continue?
					</p>
