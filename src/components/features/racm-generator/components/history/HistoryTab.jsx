...
import { getRacmJobs, deleteRacmJob } from '../../service/racm.service';
...
const HistoryTab = ({ onViewJob }) => {
	...
	const handleDelete = async () => {
		...
		try {
			setIsDeleting(true);
			await deleteRacmJob(deleteJobId);
			toast.success('Job deleted successfully');
			queryClient.invalidateQueries(['racm-jobs-history']);
		} catch {
		...
		return (
			<div className="text-center py-12">
				<p className="text-primary40 text-sm">
					No RACM jobs yet. Generate your first RACM from the Generator
					tab.
				</p>
			</div>
		);
			...
			<Dialog open={!!deleteJobId} onOpenChange={() => setDeleteJobId(null)}>
				<DialogContent className="max-w-sm">
					...
					<p className="text-sm text-primary60 mt-2">
						This will permanently delete this report and cannot be
						undone. Continue?
					</p>
