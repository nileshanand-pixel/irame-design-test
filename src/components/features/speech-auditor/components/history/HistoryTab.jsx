...
const HistoryTab = ({ onViewJob }) => {
	...
	const { data: jobs, isLoading } = useQuery({
		queryKey: ['sa-jobs-history'],
		queryFn: getSpeechAuditorJobs,
	...
	const handleDelete = async () => {
		...
		try {
			setIsDeleting(true);
			await deleteSpeechAuditorJob(deleteJobId);
		...
		return (
			<div className="text-center py-12">
				<p className="text-primary40 text-sm">
					No Speech Auditor jobs yet. Upload a call recording to get
					started.
				</p>
			</div>
		);
