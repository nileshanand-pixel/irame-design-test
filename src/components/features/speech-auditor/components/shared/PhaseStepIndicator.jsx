import { SA_STAGES, SA_STAGE_ORDER } from '../../constants/speechAuditor.constants';

const PhaseStepIndicator = ({ currentStage, status }) => {
	const currentIndex = SA_STAGE_ORDER.indexOf(currentStage);
	const isCompleted = status === 'COMPLETED';

	return (
		<div className="flex items-center justify-between w-full px-2">
			{SA_STAGE_ORDER.map((stageKey, index) => {
				const stage = SA_STAGES[stageKey.toUpperCase()] || {
					label: stageKey,
					color: 'text-gray-500',
					bgColor: 'bg-gray-500',
				};
