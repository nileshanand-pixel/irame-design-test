...
export default function useS3File() {
    ...
    const openS3File = async (s3Url) => {
        try {
            ...
            toast.error(`Error opening file: ${error?.response?.data?.message || error?.message}`);
            console.log(error);
        } finally {
            setIsOpening(false);
        }
    ...
    const createS3File = async (s3Url) => {
        try {
            ...
            toast.error(`Error creating signed file url: ${error?.response?.data?.message || error?.message}`);
        } finally {
